let Content,mongoose,Joi,sanitizeHtml;_1df‍.x([["getContentById",()=>getContentById],["checkOwnContent",()=>checkOwnContent],["write",()=>write],["list",()=>list],["read",()=>read],["remove",()=>remove],["update",()=>update]]);_1df‍.w("../../models/content",[["default",["Content"],function(v){Content=v}]]);_1df‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_1df‍.w("joi",[["default",["Joi"],function(v){Joi=v}]]);_1df‍.w("sanitize-html",[["default",["sanitizeHtml"],function(v){sanitizeHtml=v}]]);//ContentSchema를 이용해 만든 Content 모델 객체 불러옴.
...
//특정 id를 갖는 content 삭제
  ...
  const result = Joi.validate(ctx.request.body, schema);
  if (result.error) {
    _1df‍.g.console.log(result.error);
    ctx.status = 400; //bad request
    ctx.body = result.error;
    return;
  }
