let Content,mongoose,Joi,sanitizeHtml;_88a‍.x([["getContentById",()=>getContentById],["checkOwnContent",()=>checkOwnContent],["write",()=>write],["list",()=>list],["fullList",()=>fullList],["read",()=>read],["remove",()=>remove],["update",()=>update]]);_88a‍.w("../../models/content",[["default",["Content"],function(v){Content=v}]]);_88a‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_88a‍.w("joi",[["default",["Joi"],function(v){Joi=v}]]);_88a‍.w("sanitize-html",[["default",["sanitizeHtml"],function(v){sanitizeHtml=v}]]);//ContentSchema를 이용해 만든 Content 모델 객체 불러옴.
...
const { ObjectId } = mongoose.Types;
       ...
       const getContentById = async (ctx, next) => {
  const { id } = ctx.params;
...
//자신이 작성한 작품인지 결정. 관리자일 경우에도 true
       const checkOwnContent = (ctx, next) => {
...
//포스트 작성, async로 비동기 처리.
  ...
  _88a‍.g.console.log(result.value);
...
//특정 id를 갖는 content 삭제
  ...
  const result = Joi.validate(ctx.request.body, schema);
  if (result.error) {
    _88a‍.g.console.log(result.error);
    ctx.status = 400; //bad request
    ctx.body = result.error;
    return;
  }
